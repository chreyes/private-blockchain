...
class BlockchainController {
    ...
    // Enpoint to Get a list of errors in case the blockchain is invalid.
    getChainValidated() {
        this.app.get("/validateBlockchain", async (req, res) => {
            let listErrors = await this.blockchain.validateChain();
            if (listErrors.length > 0) {
                return res.status(404).json(listErrors);
            } else {
                return res.status(200).json('No errors found in the blockchain!');
            }
        });
    }
